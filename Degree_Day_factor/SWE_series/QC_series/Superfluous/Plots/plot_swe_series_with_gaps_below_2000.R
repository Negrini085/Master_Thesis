...
plot_swe_comparison <- function(hs_series, swe_from_model, station_name, year, ele, end_zero, start_zero, max_swe, max_hs) {
  ...
  p1 <- ggplot(df_hs, aes(x = date)) +
    geom_rect(
      data = df_hs[df_hs$gap_flag, ], aes(xmin = previous_date, xmax = next_date, ymin = 0, ymax = Inf),
      fill = "#C1FFC1", alpha = 1, color = NA, linewidth = 0, inherit.aes = FALSE
    ) + 
    geom_area(aes(y = value_pre, group = group_id),    fill = "red",    alpha = 0.2) +
    geom_area(aes(y = value_season, group = group_id), fill = "grey40", alpha = 0.2) +
    geom_area(aes(y = value_post, group = group_id),   fill = "red",    alpha = 0.2) +
...
for(name in station_names[1:3]){
  ...
  for(y in unique(as.numeric(merged_df$year))){
    ...
    suppressWarnings({
      ...
      ggsave(paste0("Images/with_gaps/", name, "_", y-1,"_to_", y, ".pdf"),
             plot = p, width = 12, height = 10, version = cairo_pdf)
    })
