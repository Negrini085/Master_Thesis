...
split_series_com <- function(df, end_zero, start_zero) {
  n     <- nrow(df)
  dates <- df$date
...
# Function to split a dataframe into pre-season, season and post-season series for an incomplete hydrological year
split_series_gap <- function(df, end_zero, start_zero, appo_gaps){
  n <- nrow(df)
  dates <- df$date
...
plot_swe_comparison_com <- function(hs_series, swe_from_hs, swe_from_model, station_name, year, ele, end_zero, start_zero, max_swe, max_hs) {
  ...
  df_hs        <- split_series_com(df_hs,        end_zero, start_zero)
  df_swe_hs    <- split_series_com(df_swe_hs,    end_zero, start_zero)
  df_swe_model <- split_series_com(df_swe_model, end_zero, start_zero)
...
# Function to create comparison plot between HS, SWE from DeltaSnow and SWE from model for an incomplete hydrological year
plot_swe_comparison_gap <- function(hs_series, swe_from_model, station_name, year, ele, end_zero, start_zero, max_swe, max_hs) {
  ...
  # Split each series into pre, season and post
  df_hs <- split_series_gap(df_hs, end_zero, start_zero, as.numeric(df_hs$value))
  df_swe_model <- split_series_gap(df_swe_model, end_zero, start_zero, as.numeric(df_swe_model$value))
...
for(name in unique(df_tot$name)){
  ...
  for(i in seq_along(total_years)){
    ...
    if(total_years[i] %in% c(2024, 2025)) next
