...
df_plot <- data.frame(lon = as.numeric(appo$lon), lat = as.numeric(appo$lat), status = ifelse(appo$mark == "ok", "OK", "NO"))
...
ggplot() +
  ...
  theme_minimal() +
  labs(title = "AWS Station reliability analysis: 15 meters threshold", x = "Longitude", y = "Latitude") +
