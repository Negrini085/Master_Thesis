...
fname_not_consider_1 <- "Datas/MOD200_STA350.dat"
fname_not_consider_2 <- "Datas/MOD200_STA300.dat"
fname_not_consider_3 <- "Datas/MOD250_STA300.dat"
...
df <- read.table(fname_not_consider_3)
name_no_consider_3 <- unique(df$V1)
...
mask <- merged$los_MOD < 100 & merged$los_STA > 200
...
to_analyze <- to_analyze[!(to_analyze$name %in% name_no_consider_3), ]

write.table(to_analyze, "Datas/MOD100_STA200.dat", row.names = FALSE, col.names = FALSE, quote = FALSE)