...
normal_to_hydro <- function(station_name, start_year, end_year){
  ...
  fname <- paste0("Dataset/modis_series/", station_name)
  ...
  write.table(df, paste0("Dataset/modis_hydrological/", station_name) , row.names = FALSE, col.names = FALSE, quote = FALSE)
}
