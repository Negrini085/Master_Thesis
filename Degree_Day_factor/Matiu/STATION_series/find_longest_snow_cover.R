# The main goal of this script is to compute the duration of persistent snow cover
# at a certain station. I would like to make use as much as possible of the code
# previously developed for handling MODIS series.
rm(list = ls())
gc()
...
find_longest_sc_period_station <- function(station_name, mark){
  ...
  if(mark == "NO") fname <- paste0("Dataset/hydro_sc_series/non_compatible/", station_name)
  else fname <- paste0("Dataset/hydro_sc_series/compatible/", station_name)
