...
all_mark_series <- character(0)

for(i in seq_len(length(station_names))){
  
  # Evaluating start position and snow cover series
  eff_start <- max(start_year[i], 2001)
  if(end_year[i] < 2001) print("Operational window problem!")
  ...
  all_mark_series <- c(all_mark_series, rep(mark[i], length(appo)))
  all_years_series <- c(all_years_series, seq(eff_start, end_year[i]))
...
df_los <- data.frame(
  ...
  appo_m = all_mark_series
)
write.table(df_los, "Results/los_from_annual_modis_maps.dat", row.names = FALSE, col.names = FALSE, quote = FALSE)