...
normal_to_hydro <- function(station_name, start_year, end_year, mark){
  ...
  if(mark == "NO"){
    fname <- paste0("Dataset/sc_series/non_compatible/", station_name)
    fout <- paste0("Dataset/hydro_series/non_compatible/", station_name)
  }
  else{
    fname <- paste0("Dataset/sc_series/compatible/", station_name) 
    fout <- paste0("Dataset/hydro_series/compatible/", station_name) 
  }
...
for(i in seq_along(station_names)){
  normal_to_hydro(station_names[i], start_year[i], end_year[i], mark[i])
}