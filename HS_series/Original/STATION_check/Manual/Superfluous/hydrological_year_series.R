...
fname <- "Dataset/solar/ANAGRAFICA_MANUALE_TOT"
...
normal_to_hydro <- function(station_name){
  ...
  # the dataset.
  fname <- paste0("Dataset/solar/", station_name)
  ...
  if(nrow(df) != 2){
    ...
    for(i in seq_len(n_iter)){
      ...
      mask <- is.na(appo) | appo == -90.0 | appo == 0
      if(all(mask)) rows_to_remove <- c(rows_to_remove, ind:(ind+11))
    }
    ...
    # Saving to file
    write.table(df, paste0("Dataset/hydro/", station_name), row.names = FALSE, col.names = FALSE, quote = FALSE)
    return(1)
...
for(i in seq_along(station_names)){
  ...
  if(appo == 1) usable_stations <- c(usable_stations, station_names[i])
}

df_print <- data.frame(station_names = usable_stations)
write.table(df_print, "Dataset/hydro/hydro_stations.dat", row.names = FALSE, col.names = FALSE, quote = FALSE)