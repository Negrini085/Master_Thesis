...
coord_ele <- as.matrix(data.frame(lon = as.numeric(appo$lon), lat = as.numeric(appo$lat), ele = as.numeric(appo$ele)), ncol = 3)
station_names <- appo$name
rm(appo)
gc()
...
write.table(results, file = "Dataset/check_ele.dat", row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")