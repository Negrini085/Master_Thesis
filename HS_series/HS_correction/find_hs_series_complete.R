...
for(i in seq_along(station_names)){
  ...
  for(y in single_y){
    ...
    # Skipping years which don't have a model SWE series
    ...
    if(all(!is.na(appo_hs) & appo_hs >= 0)){
...
write.table(df_print, "Dataset/hs_series/all_complete/all_complete.dat", row.names = FALSE, col.names = FALSE, quote = FALSE)