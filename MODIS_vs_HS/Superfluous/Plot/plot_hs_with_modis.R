...
fname <- "Results/filtered_los.dat"
...
for(name in unique(station_names)){
  ...
  fname_mod <- paste0("../HS_series/Correct/MODIS_series/Dataset/modis_hydrological/", name)
  if(!file.exists(fname_mod)) stop(paste0("No MODIS series for ", name))
  df_mod <- read.table(fname_mod)
  
  fname_swe <- paste0("../HS_series/HS_correction/Dataset/model_runs/hydro/SNWD/DV_SDH_", sub("HSD_", "", name))
