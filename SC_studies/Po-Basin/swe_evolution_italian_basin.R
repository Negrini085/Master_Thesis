# The main goal of this script is to determine swe evolution of the snowpack only
# on the italian part of Po Basin. We will make use of linear interpolation in order 
# to overcame dataset gaps.
...
# Importing mask to sample only from Italian territory
mask_Italy <- rast("Dataset/Italy_Mask.tif")
appo <- rast("Dataset/1992/SWE_1991-10-03.tif")
...
for(y in years){
  ...
  for(i in 1:length(months)){
    ...
    for(d in 1:dayMax[i]){
      ...
      if(file.exists(appo)){
        ...
        # Here I mask pixel outside of Italian territory
        maskP <- !is.na(mask_Italy)
        appo <- mask(area*swe_tif*10^-12, maskP, maskvalues = FALSE)
        total_swe <- global(appo, fun="sum", na.rm = TRUE)$sum[1]
  ...
  write.table(df$swe, file = paste0("Datas/Volume_SWE/Evo_Italian_SWE/swe_evolution", y, ".dat"), row.names = FALSE, col.names = FALSE)
...
write.table(df$swe, file = "Datas/swe_italian_po_evolution.dat", row.names = FALSE, col.names = FALSE)
