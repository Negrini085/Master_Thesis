...
make_plot <- function(layer_idx, year, show_x = FALSE, show_y = FALSE) {
  ...
  ggplot(grid, aes(x = E, y = N, fill = swe)) +
    ...
    scale_fill_viridis_c(
      option = "plasma", direction = 1, name = "SWE [m w.e]",
      limits = c(0, 365), oob = squish
    ) +
    coord_equal() +
...
for (p in 1:4) {
  ...
  fname_out <- paste0("Images/scd_maps/scd_maps_", years[idx_start], "_", years[idx_end], ".png")
