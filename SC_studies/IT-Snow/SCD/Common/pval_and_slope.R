...
fnames <- c("Datas/pval_map.nc", "Datas/slope_map.nc")
...
# Importing raster. Clipping datas on dem
los_metrics <- rast(fnames)
los_metrics$pval <- ifel(is.na(los_metrics$pval), 1, los_metrics$pval)


dem <- rast("Datas/DEM_Italy.tif")
ext(dem) <- ext(los_metrics)
los_metrics$pval <- ifel(is.na(dem), NA, los_metrics$pval)
los_metrics$Slope <- ifel(is.na(dem), NA, los_metrics$Slope)

# Creating a positive/negative pval map in order to distinguish between positive 
# and negative trend as it was done in Italian MODIS paper.
print(global(los_metrics$pval, "min", na.rm = TRUE)$min)
print(global(los_metrics$pval, "max", na.rm = TRUE)$max)
los_metrics$pval <- ifel(los_metrics$Slope < 0, -1+los_metrics$pval, 1 - los_metrics$pval)
