...
fnames <- c("Datas/scd_std_map.nc", "Datas/scd_mean_map.nc")
...
# Importing raster/dem and filtering over dem
...
mask_dem <- is.na(dem)
los_metrics$std <- mask(los_metrics$std, mask_dem, maskvalue = 1)
los_metrics$mean <- mask(los_metrics$mean, mask_dem, maskvalue = 1)
...
p1 <- make_snow_plot(
  los_metrics[[1]], "Standard deviation SCD",
...
p2 <- make_snow_plot(
  los_metrics[[2]], "Noise over signal SCD",
