...
sweElevation <- function(swe, dem, bands){
  ...
  for(i in 1:length(bands)){
    ...
    if(i == 1){
      maskP <- dem < bands[i] & !is.na(swe)
    }
    else{
      maskP <- dem < bands[i] & dem >= bands[i-1] & !is.na(swe)
    }
  ...
  for(i in 1:length(months)){
    ...
    for(d in 1:dayMax[i]){
      ...
      if(file.exists(appo)){
        ...
        # Storing swe value
        swe_year[conta, ] <- sweElevation(swe = swe, dem = dem, bands = bands)
        conta <- conta + 1
      }
...
write.table(df, file = "Datas/swe_evolution_elevation.dat", row.names = FALSE, col.names = FALSE)