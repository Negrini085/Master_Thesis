# The main goal of this script is to create a plot to show the difference between
# snow stored on the whole Po Basin and only on its italian part.
rm(list=ls())
gc()
...
pippo <- read.table("Datas/swe_italian_po_evolution.dat")
pippo <- pippo$V1
...
for(y in years){
  if(y == 1992){
    swe_evolution <- appo[1:(dur+1)] - pippo[1:(dur+1)]
    conta <- conta + dur + 1
  }
  else if(y%%4 == 0){
    swe_evolution <- c(swe_evolution, appo[(conta+1):(conta+dur+1)] - pippo[(conta+1):(conta+dur+1)])
    conta <- conta + dur + 1
  }
  else{
    swe_evolution <- c(swe_evolution, appo[(conta+1):(conta+dur)] - pippo[(conta+1):(conta+dur)])
    conta <- conta + dur
  }
...
ggplot(df, aes(x = day, y = swe)) + 
  ...
  labs(title = "Difference between whole Po Basin and only Italian part", x = "Days", y = "SWE Gm^3") +
  theme_minimal() +
