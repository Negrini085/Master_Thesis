...
L'approccio scelto per modellizzare lo SWE del manto nevoso è raster-based one
The chosen modelling approach is a raster-based one: adjacent pixels were treated as independet, neglecting every horizontal transport mechanism,. Snowfall and snowmelt were computed on a daily basis, for estimating the changes in water storage spatial distribution 
across the investigated domain. 
\subsection{Snow accumulation}

The occurance of snowfall was determined on a daily basis using a temperature threshold method. In particular, the minimum and maximum daily 
temperatures were compared, for each grid point, with the threshold one $T_\tau$, in order to compute the solid precipitation fraction. If a 
